...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        ...
        # 合并分类，原有分类优先（若已存在则保留原值，新分类仅追加）
        for k, v in new_categories.items():
            if k not in cateManual:
                cateManual[k] = v
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        try:
            ...
            # 解析分页，获取最大页码
            max_page = 1
            page_links = root.xpath('//div[@class="page paginate"]//a[@class="btn page-btn"]/@href')
            for link in page_links:
                match = re.search(r'[?&]idx=(\d+)', link)
                if match:
            ...
            result['list'] = vodList
            result['page'] = pg
            result['pagecount'] = max_page if max_page > 1 else 1  # 至少为1
            result['limit'] = 90
            result['total'] = max_page * 90  # 估算总数
        except Exception as e:
