...
class Spider(BaseSpider):
    ...
    HOSTS = [
        'https://www.222054.xyz',   # 黄色仓库镜像，完全可用
        'http://68ck.net',          # 跳转壳，需JS解析
        'https://hscangku.com',
        'https://456260.xyz',
        'https://567955.xyz',
        'http://hsck.net',
    ...
    def _resolve_jump_shell(self, url, html):
        """解析 68ck.net 这类 JS 跳转壳"""
        if 'strU=' not in html and 'hao123' not in html:
            return None
        ...
        jump_url = f'{template}{current}&p={path}'
        self._log(f'检测到跳转壳: {url} -> {jump_url}')
        return jump_url
        ...
        for host in self.HOSTS:
            try:
                ...
                if r.status_code == 200 and len(r.text) > 2000:
                    ...
                    # 检查是否是跳转壳
                    jump_url = self._resolve_jump_shell(r.url, r.text)
                    if jump_url:
                        # 尝试跟随跳转
                        r_jump = self.session.get(jump_url, timeout=10, verify=False, allow_redirects=True)
                        if r_jump.status_code == 200 and len(r_jump.text) > 2000:
                            final_host = r_jump.url.rstrip('/')
                            self._log(f'跳转壳跟随成功: {final_host}')
                        else:
                            self._log(f'跳转壳目标不可用: {jump_url}')
                            continue
                    # 测试内页
                    test_url = f'{final_host}/vodtype/1-1.html'
                    r2 = self.session.get(test_url, timeout=8, verify=False, allow_redirects=True)
                    if r2.url.rstrip('/').endswith(('.com', '.xyz', '.net', '.us', '.cc', '.space')) and urlparse(r2.url).path in ('', '/'):
    ...
    def _fetch(self, url, referer=None, retries=3):
        ...
        for attempt in range(retries):
            try:
                ...
                final_url = r.url
                # 处理 68ck.net 这类跳转壳
                if r.status_code == 200 and len(r.text) < 1000 and ('strU=' in r.text or 'hao123' in r.text):
                    jump_url = self._resolve_jump_shell(url, r.text)
                    if jump_url:
                        r = self.session.get(jump_url, headers=headers, timeout=15, verify=False)
                        final_url = r.url
                        self._log(f'跟随跳转壳后请求: {final_url}')
                # 检测是否被重定向到不同域名
        ...
        for h in self.HOSTS:
            ...
            try:
                ...
                if r.status_code == 200 and len(r.text) > 1000:
                    # 检查是否是跳转壳
                    if len(r.text) < 1000 and ('strU=' in r.text or 'hao123' in r.text):
                        jump_url = self._resolve_jump_shell(new_url, r.text)
                        if jump_url:
                            r = self.session.get(jump_url, headers=self._get_headers(referer or h+'/'), timeout=15)
                    if len(r.text) > 1000:
                        self.host = h if not ('strU=' in r.text and len(r.text) < 1000) else self.host
                        r.encoding = 'utf-8'
