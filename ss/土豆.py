...
class Spider(BaseSpider):
    ...
    def _detect_working_host(self):
        for host in self.HOSTS:
            try:
                r = self.session.get(host, timeout=5, verify=False)
                if r.status_code == 200 and len(r.text) > 2000:
                    self._log(f'探测成功: {host} (长度:{len(r.text)})')
                    return host
                else:
                    self._log(f'探测失败(短内容): {host} 长度:{len(r.text) if r.text else 0}')
            except Exception as e: self._log(f'探测失败: {host} - {e}')
        return self.HOSTS[0]
    ...
    def _fetch(self, url, referer=None, retries=3):
        ...
        for attempt in range(retries):
            ...
            except Exception as e: self._log(f'请求失败 [{attempt+1}]: {url} - {e}')
            time.sleep(1)
        for host in self.HOSTS:
            if host == self.host: continue
            try:
                new_url = urljoin(host, urlparse(url).path + ('?'+urlparse(url).query if urlparse(url).query else ''))
                r = self.session.get(new_url, headers=self._get_headers(referer or host+'/'), timeout=15)
                if r.status_code == 200 and len(r.text) > 1000:
                    self.host = host; r.encoding = 'utf-8'
                    self._log(f'切换域名成功: {host} -> 长度:{len(r.text)}')
                    return r.text
            except: pass
        return ''
    ...
    # ===== 列表解析（仅取 /v5/ 或 /vodplay/ 链接） =====
    def _parse_list(self, html):
        items, seen_vids = [], set()
        cards = re.findall(r'<li[^>]*>(.*?)</li>', html, re.S)
        for card in cards:
            if 'stui-vodlist__box' not in card: continue
            a_match = re.search(r'<a[^>]+href="([^"]+)"', card)
            if not a_match: continue
            href = a_match.group(1).strip()
            if not (href.startswith('/v5/') or href.startswith('/vodplay/')): continue
            ...
            if h4: title = h4.group(1).strip()
            else:
                t_attr = re.search(r'title="([^"]+)"', card)
                if t_attr: title = t_attr.group(1).strip()
                else:
                    inner = re.search(r'<a[^>]*>(.*?)</a>', card, re.S)
                    if inner: title = re.sub(r'<[^>]+>', '', inner.group(1)).strip()
            if not title: title = vid
            pic = ''
            img = re.search(r'data-original="([^"]+)"', card)
            if img:
                pic = img.group(1)
                if pic.startswith('//'): pic = 'http:' + pic
    ...
    def homeContent(self, filter):
        text = self._fetch(self.host + '/')
        if text and len(text) > 2000: self._update_categories(text)
    ...
    def homeVideoContent(self):
        if self._categories_cache: return {'list': self._get_list(self._categories_cache[0]['type_id'], 1)}
        return {'list': []}
    ...
    def detailContent(self, ids):
        ...
        if not detail:
            ...
            if not detail.get('vod_name'): detail['vod_name'] = f'视频_{vid}'
        return {'list': [detail]}
    ...
    def _parse_detail(self, html, vid):
        # 【修复1】标题提取：优先从 <title> 用 " - " 分隔提取
        title = ''
        # 方法A：从 <title> 提取，用 " - " 作为分隔符
        ...
        if m:
            ...
            if len(parts) >= 2:
                ...
                title = full_title
        # 方法B：从 h3.title 提取，排除"目录"和"精选内容"
        if not title:
        ...
        # 【修复2】参数提取：支持 var AID='x', ASID='y' 格式（多个变量在同一 var 声明中）
        aid = asid = anid = ak = ''
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        if html:
            ...
            if m: return {'parse': 0, 'url': m.group(1), 'header': {'Referer': self.host}}

        # player_aaaa
        if html:
            m3u8 = self._extract_player_aaaa(html)
            if m3u8: return {'parse': 0, 'url': m3u8, 'header': {'Referer': self.host}}

        # 【修复3】count.php 参数提取 - 支持多种格式
        aid = asid = anid = ak = ''
        if 'aid=' in id:
            try:
